...
# 数据处理函数，加入傅里叶变换特征
def create_dataset_with_fft(data, time_step=96, output_step=96, fft_components=48):
    X, y = [], []
...
def evaluate_model(model, test_loader, scaler, cishu):
    ...
    plt.legend()
    filename = f'actual_vs_predicted_bike_counts_96_hours_ff_{cishu + 1}_48.png'
...
# 训练和测试模型的函数
def train_and_evaluate_with_fft(train_cnt, test_cnt, time_step=96, output_step=96, fft_components=48, num_epochs=3,
    ...
    for exp in range(num_experiments):
        ...
        if mse < best_mse and mae < best_mae:
            ...
            best_mae = mae
            best_model = model
            torch.save(model.state_dict(), 'best_model_96_ff_48.pth')  # 保存最优模型
            ...
            torch.save(model_info, 'best_model_with_scores_96_ff_48.pth')  # 保存最优模型和相关指标
...
# 加载最优模型
def load_best_model(model_class, model_path='best_model_96_ff_48.pth'):
...
# 加载最优模型和指标
def load_best_model_with_scores(model_class, model_path='best_model_with_scores_96_ff_48.pth'):
    # 加载模型的状态字典和相关指标
...
def plot_predictions(model, test_loader, scaler):
    ...
    plt.ylabel('Bike Count (cnt)')
    plt.legend()
    plt.savefig('best_actual_vs_predicted_bike_counts_96_hours_ff_48.png')  # 保存为图片文件
    plt.close()
...
if __name__ == "__main__":
    ...
    if sel == 0:
        ...
        X_test, y_test = create_dataset_with_fft(test_cnt_scaled, time_step=96, output_step=96, fft_components=48)
