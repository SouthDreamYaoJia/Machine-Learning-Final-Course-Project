...
class Attention(nn.Module):
    ...
    def forward(self, lstm_output):
        ...
        query = lstm_output  # [batch_size, seq_len, hidden_size]
        key = lstm_output  # [batch_size, seq_len, hidden_size]
        energy = torch.matmul(query, self.W)  # [batch_size, seq_len, hidden_size]
        attention = torch.softmax(torch.matmul(energy, key.transpose(1, 2)) / self.scale, dim=-1)  # [batch_size, seq_len, seq_len]
        weighted_output = torch.matmul(attention, lstm_output)  # [batch_size, seq_len, hidden_size]
...
class LSTMCNNModel(nn.Module):
    def __init__(self, time_step, num_features, num_outputs, dropout_rate=0.2):
        ...
        self.conv1 = nn.Conv1d(in_channels=num_features, out_channels=64, kernel_size=3, padding=1)  # 保持序列长度不变
        self.relu = nn.ReLU()
        self.lstm1 = nn.LSTM(input_size=64, hidden_size=50, num_layers=1, batch_first=True)
        self.dropout1 = nn.Dropout(dropout_rate)
        self.attention = Attention(hidden_size=50)
...
for experiment in range(num_experiments):
    ...
    model = LSTMCNNModel(time_step, num_features, num_outputs).to(device)
...
# 保存最佳模型
torch.save(best_model.state_dict(), f'LSTMCNN_96_best_model_experiment_{best_experiment + 1}.pth')
...
# 写入文件
with open(f'实验结果_LSTMCNN_96_{datetime.now().strftime("%Y_%m_%d_%H_%M")}.txt', 'w') as f:
...
plt.ylabel('Count')
plt.legend()
plt.savefig(f'LSTMCNN_96_MultiValue_train_prediction_experiment_{experiment + 1}_{timeStamp}.png')
...
plt.ylabel('Count')
plt.legend()
plt.savefig(f'LSTMCNN_96_MultiValue_test_prediction_experiment_{experiment + 1}_{timeStamp}.png')