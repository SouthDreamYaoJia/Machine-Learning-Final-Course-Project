...
# 数据处理函数
def create_dataset(data, time_step=96, output_step=96):
    X, y = [], []
...
class TransformerModel(nn.Module):
    def __init__(self, input_size, output_size, nhead=8, num_encoder_layers=6, num_decoder_layers=6, hidden_dim=512):
        super(TransformerModel, self).__init__()
        self.input_size = input_size
        self.output_size = output_size

        self.encoder_layer = nn.TransformerEncoderLayer(d_model=input_size, nhead=nhead, dim_feedforward=hidden_dim,
                                                        batch_first=True)
        self.encoder = nn.TransformerEncoder(self.encoder_layer, num_layers=num_encoder_layers)

        self.decoder_layer = nn.TransformerDecoderLayer(d_model=input_size, nhead=nhead, dim_feedforward=hidden_dim,
                                                        batch_first=True)
...
def evaluate_model(model, test_loader, scaler, cishu):
    ...
    plt.plot(range(96), y_true, label='Actual', color='blue', alpha=0.6)  # 横坐标为小时，范围0-95
    plt.plot(range(96), y_pred, label='Predicted', color='red', alpha=0.6)  # 横坐标为小时，范围0-95
    plt.title('Actual vs Predicted Bike Counts (96 Hours)')
    plt.xlabel('Hour (0 to 95)')
    plt.ylabel('Bike Count (cnt)')
    plt.legend()
    filename = f'actual_vs_predicted_bike_counts_96_hours_{cishu + 1}.png'
...
# 训练和测试模型的函数
def train_and_evaluate(train_cnt, test_cnt, time_step=96, output_step=96, num_epochs=3, num_experiments=5,
                       batch_size=32):
    # 标准化数据
    scaler = MinMaxScaler()
    ...
    # 创建训练数据集和测试数据集
    X_train, y_train = create_dataset(train_cnt_scaled, time_step, output_step)
    X_test, y_test = create_dataset(test_cnt_scaled, time_step, output_step)
    ...
    for exp in range(num_experiments):
        # 初始化模型，损失函数和优化器
        model = TransformerModel(input_size=96, output_size=96)
        ...
        if mse < best_mse and mae < best_mae:
            ...
            best_mae = mae
            best_model = model
            torch.save(model.state_dict(), 'best_model_96.pth')  # 保存最优模型
            ...
            torch.save(model_info, 'best_model_with_scores_96.pth')  # 保存最优模型和相关指标
...
# 加载最优模型
def load_best_model(model_class, model_path='best_model_96.pth'):
    model = model_class(input_size=96, output_size=96)  # 初始化模型
...
# 加载最优模型和指标
def load_best_model_with_scores(model_class, model_path='best_model_with_scores_96.pth'):
    # 加载模型的状态字典和相关指标
...
def plot_predictions(model, test_loader, scaler):
    ...
    plt.ylabel('Bike Count (cnt)')
    plt.legend()
    plt.savefig('best_actual_vs_predicted_bike_counts_96_hours.png')  # 保存为图片文件
    plt.close()

    best_model, mse_scores, mae_scores = load_best_model_with_scores(TransformerModel)
...
if __name__ == "__main__":
    ...
    if sel == 0:
        # 进行训练和评估
        mse_mean, mse_std, mae_mean, mae_std = train_and_evaluate(train_cnt, test_cnt)
        ...
        X_test, y_test = create_dataset(test_cnt_scaled, time_step=96, output_step=96)
        ...
        # 加载最优模型
        model = load_best_model(TransformerModel)
