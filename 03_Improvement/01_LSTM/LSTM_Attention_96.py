...
# 保存最佳模型
torch.save(best_model.state_dict(), f'LSTM_96_best_model_experiment_{best_experiment + 1}.pth')
...
# 写入文件
with open(f'实验结果_LSTM_96_{datetime.now().strftime("%Y_%m_%d_%H_%M")}.txt', 'w') as f:
...
plt.ylabel('Count')
plt.legend()
plt.savefig(f'LSTM_96_MultiValue_train_prediction_experiment_{experiment + 1}_{timeStamp}.png')
...
plt.ylabel('Count')
plt.legend()
plt.savefig(f'LSTM_96_MultiValue_test_prediction_experiment_{experiment + 1}_{timeStamp}.png')