...
# 数据处理函数
def create_dataset(data, time_step=96, output_step=240):
    X, y = [], []
...
def evaluate_model(model, test_loader, scaler, cishu):
    ...
    plt.legend()
    filename = f'actual_vs_predicted_bike_counts_240_hours_{cishu+1}.png'
...
# 训练和测试模型的函数
def train_and_evaluate(train_cnt, test_cnt, time_step=96, output_step=240, num_epochs=3, num_experiments=5,
                       batch_size=32):
    # 标准化数据
    scaler = MinMaxScaler()
    ...
    for exp in range(num_experiments):
        # 初始化模型，损失函数和优化器
        model = TransformerModel(input_size=96, output_size=240)
        ...
        if mse < best_mse and mae < best_mae:
            best_mse = mse
            best_model = model
            torch.save(model.state_dict(), 'best_model_240.pth')  # 保存最优模型
            ...
            torch.save(model_info, 'best_model_with_scores_240.pth')  # 保存最优模型和相关指标
...
# 加载最优模型
def load_best_model(model_class, model_path='best_model_240.pth'):
...
# 加载最优模型和指标
def load_best_model_with_scores(model_class, model_path='best_model_with_scores_240.pth'):
    # 加载模型的状态字典和相关指标
...
def plot_predictions(model, test_loader, scaler):
    ...
    plt.ylabel('Bike Count (cnt)')
    plt.legend()
    plt.savefig('best_actual_vs_predicted_bike_counts_240_hours.png')  # 保存为图片文件
    plt.close()
...
if __name__ == "__main__":
    ...
    if sel == 0:
        ...
        X_test, y_test = create_dataset(test_cnt_scaled, time_step=96, output_step=240)
